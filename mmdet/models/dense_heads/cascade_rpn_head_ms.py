...
class AdaptiveConv(BaseModule):
    ...
    Args:
        ...
        type (str, optional): Type of adaptive conv, can be either 'offset'
            ...
            Default: 'dilation'.
        base_priors (int): Number of prior anchors
...
class StageMSCascadeRPNHead(RPNHead):
    ...
    def __init__(self,
        ...
        self.anchor_ratios = anchor_generator['ratios']
        self.bridged_feature = bridged_feature
        self.adapt_cfg = adapt_cfg
        super(StageMSCascadeRPNHead, self).__init__(
            in_channels,
    ...
    def _init_layers(self):
        """Init layers of a CascadeRPN stage for multi-scale anchors."""
        self.num_priors = self.prior_generator.num_base_priors[0]
        if self.adapt_cfg['type'] == 'offset':
            self.rpn_conv = nn.ModuleList([
                AdaptiveConv(self.in_channels, self.feat_channels, **self.adapt_cfg) 
            ...
            self.rpn_reg = nn.Conv2d(self.feat_channels, self.num_priors * 4, 1)
        if self.with_cls:
            if self.adapt_cfg['type'] == 'offset':
                ...
                self.rpn_cls = nn.Conv2d(self.feat_channels,
                                         self.num_priors * self.cls_out_channels, 
                                         1)
    ...
    def forward_single(self, x, offset):
        ...
        bridged_x = x.clone()
        if self.adapt_cfg['type'] == 'offset':
            assert not self.bridged_feature
            assert offset is not None
            cls_score, bbox_pred = [], []
            for i in range(self.num_priors):
                _offset = offset[:, i::self.num_priors]
                _x = self.relu(self.rpn_conv[i](x, _offset))
                _cls_score = self.rpn_cls[i](_x) if self.with_cls else None
                _bbox_pred = self.rpn_reg[i](_x)
                cls_score.append(_cls_score)
                bbox_pred.append(_bbox_pred)
            cls_score = torch.cat(cls_score, dim=1) if self.with_cls else None
    ...
    def get_targets(self,
        ...
        if isinstance(self.assigner, RegionAssigner):
            ...
            cls_reg_targets = super(StageMSCascadeRPNHead, self).get_targets(
                anchor_list,
                valid_flag_list,
                gt_bboxes,
                img_metas,
    ...
    def anchor_offset(self, anchor_list, anchor_strides, featmap_sizes):
        ...
        def _ctr_offset(anchors, stride, featmap_size):
            ...
            xx = xx.repeat(self.num_priors, 1).t().reshape(-1)  # for multi-scale priors
...
@HEADS.register_module()
class MSCascadeRPNHead(BaseDenseHead):
    ...
    def __init__(self, num_stages, stages, train_cfg, test_cfg, init_cfg=None):
        super(MSCascadeRPNHead, self).__init__(init_cfg)
