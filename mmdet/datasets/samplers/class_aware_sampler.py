...
class ClassAwareSampler(Sampler):
    ...
    def __init__(self,
        ...
        if isinstance(dataset, ConcatDataset):  # modified for RVC annotations by Feng
            def get_cat2imgs(ds):
                if ds.CLASSES is None:
                    raise ValueError('self.CLASSES can not be None')
                # sort the label index
                cat2imgs = {i + 1: [] for i in range(len(ds.CLASSES))}
                for i in range(len(ds)):
                    cat_ids = set(ds.get_cat_ids(i))
                    for cat in cat_ids:
                        if cat > 0:
                            cat2imgs[cat].append(i)
                return cat2imgs
            self.cat_dict = get_cat2imgs(dataset)
        else:
        ...
        self.valid_cat_inds = [i + 1 for i, length in enumerate(self.num_cat_imgs) if length != 0]  # re-implemented for RVC annotations by Feng
