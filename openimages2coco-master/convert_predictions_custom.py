...
def parse_args():
    ...
    parser.add_argument('-p', '--predictions', dest='predictions',
                        ...
                        type=str)
    parser.add_argument('-i', '--image_dir', dest='image_dir',
                        default=None,
                        help='path to images',
                        type=str)
...
for pred in tqdm(predictions, desc='Converting predictions '):
    ...
    img_pred_map[image_id].append([cat, str(round(conf, 4)), str(round(xmin, 4)), str(round(ymin, 4)), str(round(xmax, 4)), str(round(ymax, 4))])

# collect into per image strings
converted_predictions = [['ImageID', 'LabelName', 'Score', 'XMin', 'YMin', 'XMax', 'YMax']]
for image_id in image_ids:
    results = img_pred_map[image_id]
    for p in results:
        converted_predictions.append([image_id, p[0], p[1], p[2], p[3], p[4], p[5]])

assert len(predictions) == len(converted_predictions) - 1
